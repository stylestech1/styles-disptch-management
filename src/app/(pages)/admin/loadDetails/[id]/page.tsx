...
const LoadDetailsPage = () => {
  ...
  const [notes, setNotes] = useState<any[]>([]); // إضافة state للملاحظات
  const theme = useTheme();

  const { data, isLoading, isError, error } = useGetLoadByIdQuery(loadId);
  ...
  const handleAddNote = () => {
    if (newNote.trim()) {
      const newNoteObj = {
        ...
        text: newNote,
        content: newNote,
        addedBy: "User", // يمكنك تغيير هذا حسب المستخدم الحالي
        createdAt: new Date().toISOString(),
        type: "user_note"
      };
      
      // إضافة الملاحظة الجديدة إلى بداية القائمة
      setNotes(prevNotes => [newNoteObj, ...prevNotes]);
      setNewNote(""); // مسح حقل الإدخال
    }
  };
  ...
  if (isError) {
    const message =
      "data" in (error as any)
        ? (error as any).data?.message || "Failed to load details"
        : "Failed to load details";
    return <Erros message={message} />;
  }
  ...
  // دمج الملاحظات من الـ API مع الملاحظات المضافة محلياً
