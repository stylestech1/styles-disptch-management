...
const LoadSummary = () => {
  ...
  const applyFilter = useCallback(() => {
    ...
    const filteredSummary = originalLoadSummary.map(summary => {
      ...
      const totalMiles = filteredLoads.reduce((sum, load) => sum + (load.distanceMiles || 0), 0);
      const totalEarnings = filteredLoads.reduce((sum, load) => sum + (load.totalPrice || 0), 0);
  ...
  // ✅ دالة تطبيق الفلترة يدوياً (اختيارية)
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3">
          ...
          <div className="flex gap-2 mt-2 sm:mt-6">
            <button
              ...
              className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 disabled:bg-slate-400 disabled:cursor-not-allowed transition-colors flex items-center gap-2 text-sm"
            >
